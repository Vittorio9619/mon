...
if 10 < interval / 60 % 100 < 20:
    ...
    ending = ""
if mincost in yes:   
    cost_url = f"https://saratov.tele2.ru/api/exchange/lots/stats/volumes?trafficType={trafficType}"
    try:
...
try:
    response = requests.get(f"https://saratov.tele2.ru/api/exchange/lots?trafficType={trafficType}&volume={volume}&cost={cost}&limit=5000", headers=headers, timeout=5)
    data = response.json()
except (JSONDecodeError, ConnectionError, requests.exceptions.ReadTimeout):
    print(Fore.YELLOW + 'Нет связи с сервером')
...
def check(id):
    ...
    if not cached_data:
        try:
            response = requests.get(f"https://saratov.tele2.ru/api/exchange/lots?trafficType={trafficType}&volume={volume}&&limit=500", headers=headers, timeout=5)
...
while True:
    ...
    try:
        response = requests.get(f"https://saratov.tele2.ru/api/exchange/lots?trafficType={trafficType}&volume={volume}&cost={cost}&limit=300", headers=headers, timeout=5)
        data = response.json()
    except (JSONDecodeError, ConnectionError, requests.exceptions.ReadTimeout):
